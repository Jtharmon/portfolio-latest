...
import { ArrowLeft, Save, Upload, X, Plus, ExternalLink, Github, Trash2, Lock } from 'lucide-react';
...
function EditProject() {
  ...
  const fetchProject = async () => {
    try {
      ...
      const { blog_secret, ...projectData } = response.data;
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${API_URL}/api/projects/${id}?blog_secret=${encodeURIComponent(secretKey)}`);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-blue-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">Please authenticate to edit this project.</p>
          <button
  ...
  return (
    <>
      ...
      <SecretKeyModal
        ...
        title="Authenticate to Edit Project"
      />
    </>
  );
}
